...
const TopTracks = ({
  ...
  const artistWithMoreThanHalf = Object.entries(groupBy(tracks, (track) => track.artist)).find(
    ([_, tracks]) => tracks.length / 10 > 0.5,
  )?.[0];

  return (
    <BaseCard className={"flex flex-col p-2 rounded-3xl w-full"}>
      <span
        className={"flex items-center justify-center text-2xl font-bold text-highlightHeader m-2"}
      >
        My Spotify Top Tracks
      </span>
      {tracks ? (
        <ExpandableList
          items={tracks.map((track) => <Track {...track} key={track.id} className={"w-full"} />)}
      ...
      {artistWithMoreThanHalf ? (
        <div className="font-serif text-md text-center mt-3">
          Yeah, I know it&apos;s embarrassing to have more than half of my top ten be from{" "}
          <span className="italic">{artistWithMoreThanHalf}</span> 🙃
        </div>
      ) : null}
    </BaseCard>
  );
