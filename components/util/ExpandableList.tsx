...
const ExpandableList: React.FC<ExpandableListProps> = ({ 
  ...
  const sharedButtonStyles = "rounded-full text-link py-1 px-2 disabled:opacity-30 disabled:cursor-auto";
  const { arrowWidth, arrowHeight } = typeof arrowSize === "number"
  ...
  const moreButton = (
    ...
    >
      <Image src={'/down-arrow.svg'} height={arrowHeight} width={arrowWidth} alt="Down arrow" />
    </button>
  );
  ...
  const fewerButton = (
    ...
    >
      <Image src={'/up-arrow.svg'} height={arrowHeight} width={arrowWidth} alt="Up arrow" />
    </button>
  );
  ...
  return (
    <>
      <ol className={`${joinClasses(additionalClassNames)} flex flex-col m-1 items-center justify-center`}>
